...
Nsamples = Nbins**2
alpha = 9.0
angles = np.array(list(range(Nangles)))*2.0*np.pi/Nangles
...
def gen_dgauss(sigma):
    ...
    G = np.array(list(range(-fwid,fwid+1)))**2
...
class DsiftExtractor:
    ...
    def process_image(self, image, positionNormalize = True,\
        ...
        offsetH = int(remH/2)
        offsetW = int(remW/2)
        gridH,gridW = np.meshgrid(list(range(offsetH,H-pS+1,gS)), list(range(offsetW,W-pS+1,gS)))
        gridH = gridH.flatten()
        gridW = gridW.flatten()
        if verbose:
...
def dense_sift_multiscale(im, scales=SCALES):
    sigma_edge = 0.8
    features = []
    positions = []
    for scale in scales:
        sift = dsift.DsiftExtractor(4,24, sigma_edge=scale)
