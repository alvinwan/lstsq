...
def conv_and_upload_blocks(X_sharded, X_lift_sharded, filters_mmap, conv_args, blocks_0, outfeatures, imshape=(3, 256, 256), num_out_blocks=1):
    ...
    for block_0 in blocks_0:
        ...
        #TODO: Currently only 1 out block supported

        block_exists = (block_0, 0) in blocks_exist
        ...
        def LOGINFO(msg):
            logger.info(msg, extra={"gpu":gpuid})
        ...
        if (block_exists):
            print("Block exists so skipping")
            LOGINFO("Output row {0} exists skipping".format(block_0))
            continue
        else:
            print("Block does not exist so convolving")
...
def reset_msg_visibility(msg, not_done):
    print("Starting message visbility resetter..")
    while(not_done[0]):
        time.sleep(60)
        print("RESETTING MESSAGE VISIBILITY.. for message {0}".format(msg.body))
...
if __name__ == "__main__":
    ...
    while(True):
        ...
        not_done = [True]
        executor.submit(reset_msg_visibility, message, not_done)
