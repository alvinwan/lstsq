...
def conv_and_upload_blocks(X_input, X_output, big_patches_sharded, conv_args, row_blocks_in,  col_block_out, col_block_size, imshape=(3, 32, 32),overwrite=False):
    import os
    os.system("sudo nvidia-smi -pm 1")
    boto3.setup_default_session(region_name='us-west-2')
    gpuid = "gpu" + str(os.environ.get("CUDA_VISIBLE_DEVICES"))
    patch_path = "/dev/shm/{0}".format("patches_{0}({1})".format(conv_args['patch_size'], big_patches_sharded.key))
    if (not os.path.isfile(patch_path)):
        ...
        patches = coates_ng_help.grab_patches(big_patches, tot_patches=1e6, patch_size=conv_args['patch_size'], seed=0)
        filters_mmap_data = np.memmap(patch_path, shape=patches.shape, dtype='float32', mode="w+")
        np.copyto(filters_mmap_data, patches)
    else:
    ...
    for row_block in row_blocks_in:
        ...
        if (((row_block, col_block_out) in blocks_exist) and (not overwrite)):
            continue
        # Download row block
        X_block = D.get_row_block(X_input, row_blocks=[row_block], dtype="float64",mmap_loc="/dev/shm/{0}".format(gpuid))
        X_block = X_block.reshape((X_block.shape[0],) + imshape)
        conv_args_copy["data"] = X_block

        log_key = X_output.key.replace("(", "__").replace(")", "__")
        cw_handler = watchtower.CloudWatchLogHandler("cifarfeaturize")

        logfmt = 'DataBlock({0},{1})-%(levelname)s-%(message)s'.format(row_block, col_block_out)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--patches_name', type=str, help="name of patches sharded matrix", default="cifar_train")
    parser.add_argument('--out_matrix_name', type=str, help="should be of form featurized(sample_strategy_years)", default=None)
    parser.add_argument('--in_bucket', type=str, help="S3 bucket prefix where the pixel matrix lives", default="pictureweb")
    parser.add_argument('--out_bucket', type=str, help="S3 bucket prefix where the feature matrix should go", default="pictureweb")
    parser.add_argument('--in_prefix', type=str, help="S3 bucket prefix", default="pywren.linalg/")
    parser.add_argument('--out_prefix', type=str, help="S3 bucket prefix", default="pywren.linalg/")
    ...
    parser.add_argument('--num_col_blocks',type=int, help="number of column blocks", default=1)
    parser.add_argument('--feature_batch_size',type=int, help="number of features = 2*num_feature_batches*feature_batch_size",default=128)
    parser.add_argument('--patch_size',type=int, help="random patches will be of shape patch_size*patch_size",default=6)
    ...
    conv_args = \
    {
        ...
        "random_seed": 0,
        "num_column_blocks": args.num_col_blocks,
    ...
    print(out_shape_1)
    print(patches_name)
    big_patches_sharded = ShardedMatrix(patches_name, bucket=args.in_bucket)
    conv_args_copy = conv_args.copy()
    conv_args_copy["data"] = np.zeros((1,3,32,32))
    out_shape = _conv.conv_compute_output_shape(**conv_args_copy)
    d_block = out_shape[-1] * out_shape[-2] * out_shape[-3]
    d = d_block * args.num_col_blocks
    print("COlUMN BLOCK SHAPE ", d_block)
    print("OUTPUT SHAPE ", d)
    X_output_sharded = ShardedMatrix(out_matrix_name, prefix=args.out_prefix, bucket=args.out_bucket, shape=(out_shape_0, d), shard_size_0=X_input_sharded.shard_size_0, shard_size_1=d_block)
    ...
    config['runtime']['s3_key'] = "pywren.runtime/pywren_runtime-3.6-pictureweb_featurize.tar.gz"
