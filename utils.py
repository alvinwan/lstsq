...
def get_data_with_lengths(
        path: str,
        one_hotted: bool=False,
        n_train_episodes: int=-1) -> Tuple[np.array, np.array, np.array]:
    """Grab data from provided path."""
    X, Y, lengths = None, None, []
...
def get_data(
    ...
    return get_data_with_lengths(path, one_hotted, n_train_episodes)[:2]
